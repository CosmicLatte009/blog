...
function normalizeFileName(fileName) {
	...
	// 대괄호 안의 내용 중 제목 부분만 추출
	const titleMatch = name.match(/\[(.*?)\]/g);
...
async function handleUrlState() {
	...
	if (category) {
		handleCategory(normalizeFileName(decodeURIComponent(category)));
	} else if (menu) {
		await handleMenu(normalizeFileName(decodeURIComponent(menu)));
	} else if (post) {
		await handlePost(normalizeFileName(decodeURIComponent(post)));
	} else if (!menu && !post) {
		renderBlogList();
	} else {
		alert("잘못된 URL입니다.");
